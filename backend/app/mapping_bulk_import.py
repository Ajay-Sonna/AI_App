...
from app.dst_db.service import validate_fs_name

from app.fee_schedule_identity import (

    norm_match_key,

    primary_family_display_name,

    representative_artifact_for_family,
...
_CANON_HEADER_ALIASES = {
    ...
    "feeschedule": "state_schedule",

    "schedule": "state_schedule",

    "schedule_name": "state_schedule",
    ...
    *,
    ...
    for line in rows:
        ...
        if g is None:
            ...
            g = _Group(state_schedule=ss, artifact_id=aid, dst_fsname=dst)
    ...
    for g in groups.values():
        ...
        try:
            ...
            dst_clean = validate_fs_name(g.dst_fsname)
        ...
        row_art: Optional[Dict[str, Any]] = None

        lsk: Optional[str] = None

        schedule_name: Optional[str] = None



        if g.artifact_id is not None:

            row_art = get_artifact_by_id(int(g.artifact_id))
            ...
            if not row_art:
                ...
                errors.append(

                    {
                        ...
                        "message": f"Artifact {g.artifact_id} not found.",
            ...
            schedule_name = primary_family_display_name(row_art)

        else:

            lsk, schedule_name, resolve_err = resolve_schedule_family_by_name(
            ...
            if resolve_err or not lsk:
                ...
                errors.append(

                    {
                        ...
                        "message": resolve_err or f"No match for StateSchedule {g.state_schedule!r}.",
            ...
            row_art = representative_artifact_for_family(
            ...
            if not row_art:
                ...
                errors.append(

                    {
                        ...
                        "message": f"No saved file found for schedule family {schedule_name!r}.",
        ...
        if art_sc and art_sc != sc:
            ...
            errors.append(

                {
                    ...
                    "message": f"Artifact belongs to state {art_sc}, not {sc}.",
        ...
        rep_aid = row_art.get("artifact_id")

        applied_entry: Dict[str, Any] = {

            "artifact_id": int(rep_aid) if rep_aid is not None else None,

            "schedule_name": schedule_name or g.state_schedule,
        ...
        try:
            ...
            applied_entry["mapping_id"] = int(saved.get("mapping_id"))
            ...
            errors.append({"row": min(g.source_rows) if g.source_rows else 0, "message": str(ex)})
